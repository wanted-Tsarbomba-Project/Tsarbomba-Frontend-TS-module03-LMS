...
const problemDetailClasses = {
  ...
  problemBox:
    "min-w-0 flex-1 rounded-base border border-border-light bg-bg-box p-4 [&_h2]:mt-0 [&_h2]:mb-2.5 [&_h2]:text-title-lg [&_h2]:font-bold [&_h2]:text-text-primary",
  solveBox:
    "min-w-0 flex-1 rounded-base border border-border-light bg-bg-box p-4 [&_h2]:mt-0 [&_h2]:mb-2.5 [&_h2]:text-title-lg [&_h2]:font-bold [&_h2]:text-text-primary",
  problemContent:
    "whitespace-pre-wrap text-body leading-normal text-text-primary",
  ...
  bottomPanel:
    "min-h-[140px] w-full overflow-y-auto rounded-base border border-border-light p-3 text-body text-text-primary",
  ...
  submitButton:
    "h-11 min-w-[120px] cursor-pointer rounded-base border border-button-blue-bg bg-button-blue-bg text-body font-semibold text-text-white hover:not-disabled:bg-button-blue-hover-bg disabled:cursor-not-allowed disabled:opacity-60",
  chatPanel:
...
export default function ProblemChatPanel({
  ...
  return (
    <aside
      className={`${problemDetailClasses.chatPanel} ${
        chatOpen ? problemDetailClasses.open : problemDetailClasses.closed
      }`}
    >
      ...
      <div className={problemDetailClasses.chatMessages}>
        {chatMessages.map((message, index) => (
          <div
            className={`${problemDetailClasses.chatMessageWrap} ${
              message.role === "USER"
                ? problemDetailClasses.userMessageWrap
                : ""
            }`}
          ...
          >
            <div
              className={`${problemDetailClasses.chatMessage} ${
                message.role === "USER"
                  ? problemDetailClasses.userMessage
                  : problemDetailClasses.assistantMessage
        ...
        {chatSending && (
          <div className={problemDetailClasses.chatMessageWrap}>
            <div
              className={`${problemDetailClasses.chatMessage} ${problemDetailClasses.assistantMessage}`}
            >
              AI 응답 중입니다.
            </div>
          </div>
